...
def test_write_payload_size_matches_bulk_accounting_and_encodes_once(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert write.payload_size_bytes == stored_result_module._encoded_request_size([build_row.side_effect(write)])
    # Row key and cell encoding overhead count, not just the values.
    assert write.payload_size_bytes > sum(
        len(value.encode())
        for value in (write.extracted_features_json, write.error_traces_json, write.action_data_json)
    )
...
def test_bigtable_bulk_byte_metric_is_measured_before_the_client_clears_rows(
    ...
    expected_bytes = sum(write.payload_size_bytes for write in writes)
    ...
    assert stored_result_module._encoded_request_size(sent_rows(table)) < expected_bytes
...
def test_bigtable_reuses_one_table_and_reports_a_missing_status(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    metrics.histogram.assert_any_call('stored_execution_result.batch_bytes', 2 * write.payload_size_bytes)
