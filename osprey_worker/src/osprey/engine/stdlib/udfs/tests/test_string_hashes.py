...
from osprey.engine.stdlib.udfs.string_hashes import HashMd5, HashSha1, HashSha256, HashSha256Int, HashSha512
from osprey.engine.udf.registry import UDFRegistry
from osprey.worker._stdlibplugin.udf_register import register_udfs

pytestmark = [
    pytest.mark.use_udf_registry(UDFRegistry.with_udfs(HashMd5, HashSha1, HashSha256, HashSha256Int, HashSha512)),
]
...
def test_hash_sha256_int(execute: ExecuteFunction) -> None:
    data = execute(
        """
        ASCII = HashSha256Int(input="Over many a quaint and curious volume of forgotten lore")
        UTF8 = HashSha256Int(input="snowman-☃")
        """
    )
    assert data == {
        'ASCII': 2874102505864153775,
        'UTF8': 2079454941421963977,
    }
    assert all(0 <= value <= 2**63 - 1 for value in data.values())


def test_hash_sha256_int_is_registered_in_stdlib() -> None:
    registry = UDFRegistry.with_udfs(*register_udfs())
    assert registry.get('HashSha256Int') is HashSha256Int
